...
def kl_gaussian(p_mu, p_std, q_mu, q_std, layer_reduction='sum'):
    batch_size = p_mu.size(0)
...
def kl_out(recon_s, recon_t, nll_type):
    ...
    if nll_type == 'bernoulli':
        ...
        if GAUSSIAN_VAR_BREAK:
            recon_s_std = ones_like(recon_s_mu)
            recon_t_std = ones_like(recon_t_mu)
        else:
